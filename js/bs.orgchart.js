...
CreateOrgchartNodeBS.prototype = {
  ...
  addEvent: function (item) {
    ...
    //   item.addClass('active').find('[data-selector="edit-tip"]').show();
    // }).bind('mouseleave', function () {
    //   item.removeClass('active').find('[data-selector="edit-tip"]').hide();
    // });
    ...
    item.bind("mousedown", function (e) {
      var self = $(this);
      if (self.closest('.org_td').attr('rank') < 1) return; //祖先节点不移动
      //列式结构不支持拖动
      ...
      var issorgtartmove = 0;
      var handlenode = self;
      var refnode = that.dom.wrap.find("div.org_node_c").not("#" + self.attr("id")); //与移动相关的元素
      ...
      doc.data("edx", 0);
      doc.data("edy", 0);
      doc.bind('mousemove.orgchart', function (e) {
        var self = $(this);
        var ex = e.pageX || 0;
        var ey = e.pageY || 0;
        //解决IE下对象未移动BUG
        if (ex === doc.data("esx") && ey === doc.data("esy")) {
          return false;
        }
        ...
        group.css({
          "left": group.data("layout").x + self.data("edx"),
          "top": group.data("layout").y + self.data("edy")
        });
        ...
        refnode.each(function (i, node) {
          node = $(node);
          if (
            nx + nw / 2 >= node.data("layout").x && nx + nw / 2 <= node.data("layout").x + node.data("layout").w && ny >= node.data("layout").y && ny <= node.data("layout").y + node.data("layout").h
          ) {
      ...
      }).one('mouseup.orgchart', function (e) {
        var self = $(this);
        //恢复组元素样式
        ...
        doc.unbind('mousemove.orgchart contextmenu.orgchart');
        //点击非移动时收起展开
        // console.log(issorgtartmove);
        if (!issorgtartmove) that.expandNodeLayout(item);
      }).bind('contextmenu.orgchart', function () { //解决右键复制等操作导致布局错位bug
        doc.triggerHandler('mouseup.orgchart');
      });
    });
  ...
  //节点收起展开
  expandNodeLayout: function (item) {
    if (!item.nextAll('.org_table').length) {
      return false;
    }
    if (item.hasClass('collapsed')) {
      item.removeClass('collapsed').addClass('org_line_bm').nextAll('.org_table').css('visibility', '');
    } else {
      item.addClass('collapsed').removeClass('org_line_bm').nextAll('.org_table').css('visibility', 'hidden');
    }
    item.find('i.switch-flag').toggleClass('icon-minus').toggleClass('icon-add');
  },
  //计算相关元素布局信息
...
CreateOrgchartExtendBS.prototype = {
  ...
  setLine: function (pnode) {
    //创建连接线
    if (pnode.length) {
      var snode = pnode.find(".org_tr:first").children(".org_td");
      ...
      if (len) { //有子节点
        ...
        snode.each(function (i, e) {
          ...
          if (len == 1) {
            ...
            if (!e.hasClass("org_line_c"))
              e.addClass("org_line_c");
          }
        });
      } else {
